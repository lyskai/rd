# Guide to build ridehal-toolchain base docker

## Notes
The ridehal-toolchain base docker provides a basic host toolchain environment that splits from platform SDK toolchains, users can directly map the platform SDK toolchains path to this docker with '-v' option, thus the switching of platform SDK toolchains version could be easily.
...
- Step 2: build ridehal-toolchain-base docker image
  ...
  ```
    sudo docker images -a
    REPOSITORY                           TAG               IMAGE ID           CREATED         SIZE
    ridehal-toolchain-base               v1.0              84bcf57cad70       2 days ago      3.6GB
  ```
