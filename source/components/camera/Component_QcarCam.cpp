...
static uint32_t GetNumBitsOfInteger
(
    uint32_t nInteger
)
{
...
QCarCamRet_e RideHalCam::QcarcamEventCb(const QCarCamHndl_t hndl,
...
RideHalError_e RideHalCam::Init(char *pName, RideHalCamConfig_t config, Logger *pLogger)
{
    ...
    {
        RIDEHAL_DEBUG("RideHalCam::Init input id: %d, width: %d, height: %d", m_nInputId, m_nWidth, m_nHeight);
    ...
    if (RIDE_HAL_ERROR_NONE == ret)
    {
        ...
        status = QCarCamSetParam(m_QcarCamHndl, QCARCAM_STREAM_CONFIG_PARAM_EVENT_MASK, &param, sizeof(param));
        if (QCARCAM_RET_OK != status)
        {
...
RideHal_CameraFrame_t *RideHalCam::GetFrame()
{
    ...
    if (QCARCAM_RET_OK == status)
    {
        ...
        RIDEHAL_DEBUG("GetFrame index: %d ptr: %p buffer: %p, size: %d",
...
RideHalError_e RideHalCam::AllocateBuffer()
{
    ...
    if (m_nBufCnt > 0)
    {
        m_pCameraImage = new Image[m_nBufCnt];
        ...
        {
            if (RIDE_HAL_ERROR_NONE != m_pCameraImage[i].Allocate(m_nWidth, m_nHeight, m_colorFormat))
            {
            ...
            {
                if (RIDE_HAL_ERROR_NONE != m_pCameraImage[i].GetSharedBuffer(&m_pCameraFrames[i].sharedBuffer))
                {
                    RIDEHAL_ERROR("GetSharedBuffer failed");
                    ret = RIDE_HAL_ERROR_FAIL;
                    break;
                }
                else
                {
                    ...
                    m_pQcarcamBuffer[i].planes[0].memHndl = m_pCameraFrames[i].sharedBuffer.buffer.dmaHandle;
                    m_pQcarcamBuffer[i].planes[0].width = m_pCameraFrames[i].sharedBuffer.imgProps.width;
                    m_pQcarcamBuffer[i].planes[0].height = m_pCameraFrames[i].sharedBuffer.imgProps.height;
                    m_pQcarcamBuffer[i].planes[0].stride = m_pCameraFrames[i].sharedBuffer.imgProps.stride[0];
                    ...
                    RIDEHAL_DEBUG("register buffer index %d ptr: %p memHndl: %x va: %p width: %d, height: %d, steide: %d size: %d",
                                  i, &m_pCameraFrames[i], m_pQcarcamBuffer[i].planes[0].memHndl, m_pCameraFrames[i].sharedBuffer.data(),
                                  m_pQcarcamBuffer[i].planes[0].width, m_pQcarcamBuffer[i].planes[0].height, m_pQcarcamBuffer[i].planes[0].stride,
        ...
        {
            // setup buffers
            if (QCARCAM_RET_OK != (status = QCarCamSetBuffers(m_QcarCamHndl,
                                  (const QCarCamBufferList_t*)&m_qcarcamBuffers)))
            {
                RIDEHAL_ERROR("QCarCamSetBuffers error ret %d  handle %lu", status, m_QcarCamHndl);
                ret = RIDE_HAL_ERROR_FAIL;
            }
            else
            {
...
RideHalError_e RideHalCam::FreeBuffer()
{
    ...
    if (m_pCameraImage)
    {
        for (uint32_t i = 0; i < m_nBufCnt; i ++)
        {
            if (RIDE_HAL_ERROR_NONE != m_pCameraImage[i].Free())
            {
