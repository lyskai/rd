...
@details
    Generic geometric and color transformation along with some initialization
...
@param FADAS_REMAP_PIPELINE_UYVY_TO_RGB888_NORMI8
    Use map on a 8-bit UYVY image, convert the color space
    from YUV to RGB using the matrix defined FadasCvtYUV_UYVYtoRGB(), and
    also normalize output image.

@ingroup remap
****************************************************************************/
typedef enum : int32_t
{
    FADAS_REMAP_PIPELINE_1C8 = 0,                  ///< 1 8-bit channel unconverted
    FADAS_REMAP_PIPELINE_1C8_ROISCALE,             ///< 8-bit channel unconverted + ROI scaling
    FADAS_REMAP_PIPELINE_3C888,                    ///< 3 8-bit channels unconverted
    FADAS_REMAP_PIPELINE_3C888_ROISCALE,           ///< 3 8-bit channels unconverted + ROI scaling
    ...
    FADAS_REMAP_PIPELINE_UYVY_TO_RGB888_NORMI8,    ///< Convert UYVY to RGB [see FadasCvtYUV_UYVYtoRGB()] + Renormalization
    ...
    FADAS_REMAP_PIPELINE_MAX                       ///< \b WARNING: must be last
} FadasRemapPipeline_e;


/************************************************************************//**
@brief
    Fisheye distortion structure.
...
typedef struct
{
    float64_t k1; /**< First coefficient corresponding to the camera distortion model */
    float64_t k2; /**< Second coefficient corresponding to the camera distortion model */
    float64_t k3; /**< Third coefficient corresponding to the camera distortion model */
    float64_t k4; /**< Fourth coefficient corresponding to the camera distortion model */
    float64_t k5; /**< Fifth coefficient corresponding to the camera distortion model */
    float64_t k6; /**< Sixth coefficient corresponding to the camera distortion model */
    float64_t k7; /**< Seventh coefficient corresponding to the camera distortion model */
    float64_t k8; /**< Eighth coefficient corresponding to the camera distortion model */
} FadasDistCoeffs_t;
...
@brief
    ...
    geometric transformation to either an 8-bit grayscale image or an RGB888 image.
...
@details
    ...
    \image html Remap_roiScale.png width=1200px

    The upper-left corner of the ROI and the scaled ROI are the
    same. The upper-left of the ROI is assumed to be the
    upper-left (0,0) of the destination image.  However, that upper-left corner of
    the destination can be set to point elsewhere within the physical
    destination image as long as the resulting ROI did not land
...
@param src
    Pointer to the input image.  Because the map value map pixel locations are
...
@param dst
    Pointer to the output image. While not required, the destination image dimensions
    are typically the same size as the ROI. The width and stride must be equal to
    or larger than the ROI width.  The height must be equal to or larger than the ROI height.
    \n\b WARNING: Must be 128-byte aligned.
...
@param roiScale
    Optional parameter that only applies to pipelines that include ROISCALE and
    is only guaranteed for monotonically increasing maps (lens
    ...
    The scale of the destination ROI is relative to the scale of the output image.  The overall
    scale of the output map relative to the input image is determined at map
    creation [for example, FadasRemap_CreateMapFromCalib()]. This
    scale change is in addition to that scale change.  Typically
    used when feeding fixed-size ROIs into a DL network that requires fixed-sized
    input dimensions.  The upper-left corner values of the ROI remain the same.
    Only the width and height scale. The caller must adjust
...
@param normlz
    ...
    This is an optional parameter, required only when output image is to be normalized.
    \n\b WARNING:
...
****************************************************************************/
FADAS_API
FadasError_e FadasRemap_Run( FadasRemapMap_t          *map,
...
@param src
    Pointer to the input image.  Because map value map pixel locations backwards to the
    source image, the source image dimension should be equal to or larger
    than the range of map values for that dimension.
    \n\b WARNING: Must be 128-byte aligned.

@param dst
    Pointer to the output image. While not required, the destination image dimensions are typically the
    same size as the ROI. The width and stride must be equal to or larger than the ROI 
    width. The height must be equal to or larger than the ROI height.

    \n\b WARNING: Must be 128-byte aligned.

@param mapROI
    Pointer to the ROI in the output image relative to the virtual destination
    image coordinate system, which is also the map output coordinate system.
    If the mapping from the source to destination image includes a scale
    reduction (for example, 2x), the ROI must express coordinates within
    those reduced dimensions.
...
@param roiScale
    ...
    For example, a map might have a built-in scale reduction of 4.0 in each dimension.
    A value of 2.2 for this parameter means that the ROI
    dimensions are 2.2 times the output map size. Therefore, an ROI width of
    10 corresponds to a width of 22 in destination image coordinates, which
    then corresponds to a width of 88 in input image coordinates.
    \n\b WARNING: The corresponding destination image coordinates must fit
...
/************************************************************************//**
@brief
    Creates a new camera intrinsic matrix based on free scaling parameter.
...
@param newCameraProps
    Pointer to the output, which is an optimal camera matrix.

@param newWidth
    Width of the image after undistortion.

@param newHeight
    Height of the image after undistortion.

@param alpha
    Paramter for scaling. Value ranges form 0.0 to 1.0.
...
****************************************************************************/
FADAS_API
FadasError_e FadasRemap_DestroyMap( FadasRemapMap_t *map );
