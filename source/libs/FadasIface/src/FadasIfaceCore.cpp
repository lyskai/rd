...
AEEResult FadasIface_FadasCvtYUV_UYVYtoRGB( remote_handle64 handle, const uint8_t *src, int srcLen,
                                            const FadasIface_FadasImgProps_t *srcProps,
                                            uint8_t *dst, int dstLen, uint32_t dstStride )
{
...
AEEResult FadasIface_FadasCvtYUV_UYVYtoYUV( remote_handle64 handle, const uint8_t *src, int srcLen,
                                            const FadasIface_FadasImgProps_t *srcProps,
                                            uint8_t *dst, int dstLen, uint32_t dstStride )
{
    FadasImgProps_t srcImgProps;
...
AEEResult FadasIface_FadasCvtYUV_UYVYtoNV12( remote_handle64 handle, int32_t srcFd, uint32_t srcLen,
                                             uint32_t offset,
                                             const FadasIface_FadasImgProps_t *srcProps,
                                             int32_t dstFd, uint32_t dstLen, uint32_t dstStride )
{
    dspContext_t *dspContext = (dspContext_t *) handle;
...
AEEResult FadasIface_FadasCvtYUV_CreateWorkers( remote_handle64 handle, uint64_t *addr,
                                                int32_t threads,
                                                FadasIface_FadasCvtYUVPipeline_e type )
{
    int32_t pThreadsAffinity[] = { 0, 1, 2, 3 };
    void *wrkrs = FadasCvtYUV_CreateWorkers( 0, pThreadsAffinity,
                                             g_MapCvtYUVPipeline[static_cast<int>( type )] );
    *addr = (uint64_t) wrkrs;
    return AEE_SUCCESS;
}
...
AEEResult FadasIface_FadasCvtYUV_RunMT( remote_handle64 handle, uint64 addr, uint8_t *src,
...
AEEResult FadasIface_FadasRemap_CreateMapFromFisheyeCalib(
        ...
        const FadasIface_FadasDistCoeffs_t *distCoeffs, FadasIface_FadasRemapPipeline_e imgFormat,
        uint32_t mapWidth, uint32_t mapHeight, uint8_t borderConst )
{
    ...
    FadasRemapMap *map = FadasRemap_CreateMapFromFisheyeCalib(
            camWidth, camHeight, cameraProps, dCoeffs,
            g_MapImageConversion[static_cast<int32_t>( imgFormat )], mapWidth, mapHeight,
            borderConst );
    if ( map == nullptr )
    {
        return AEE_EFAILED;
...
AEEResult FadasIface_FadasRemap_Run( remote_handle64 handle, uint64 mapPtr, const uint8_t *src,
                                     int srcLen, const FadasIface_FadasImgProps_t *srcProps,
                                     uint8_t *__restrict dst, int dstLen,
                                     ...
                                     FadasIface_FadasROI_t *dstROI )
{
    ...
    {
        srcImg.plane[1] =
                const_cast<uint8_t *>( src + srcImg.props.stride[0] * srcProps->actualHeight[0] );
    }
                                       ...
                                       const FadasIface_FadasNormlzParams_t *normlz, int normlzLen )
{
    ...
    for ( int i = 0; i < srcFdsLen; i++ )
    {
        ...
        if ( FADAS_IMAGE_FORMAT_Y8UV8 == srcImg.props.format )
        {
            srcImg.plane[1] = const_cast<uint8_t *>( src[i] + srcImg.props.stride[0] *
                                                                      srcProps[i].actualHeight[0] );
        }
...
AEEResult FadasIface_FadasCvtYUV_Renormalize888( remote_handle64 handle, int32_t srcFd,
                                                 const FadasIface_FadasImgProps_t *srcProps,
                                                 ...
                                                 int32_t dstFd, uint32_t dstStride,
                                                 int32_t batchSize )
{
    dspContext_t *dspContext = (dspContext_t *) handle;
    const uint8_t *src = (uint8_t *) FadasIface_GetBufPtr( srcFd );
...
AEEResult FadasIface_FadasCvtYUV_Renormalize888u8f32( remote_handle64 handle, int32_t srcFd,
                                                      const FadasIface_FadasImgProps_t *srcProps,
                                                      const FadasIface_FadasNormlzParams_t *normlzR,
                                                      const FadasIface_FadasNormlzParams_t *normlzG,
                                                      const FadasIface_FadasNormlzParams_t *normlzB,
                                                      ...
                                                      int32_t batchSize )
{
    ...
    for ( int i = 0; i < batchSize; i++ )
    {
        ...
        qurt_mutex_lock( &dspContext->mutex );
        auto retval =
                FadasCvtYUV_Renormalize888u8f32( srcPtr, srcImgProps, normlzParamsR, normlzParamsG,
                                                 normlzParamsB, dstPtr, dstStride );
        qurt_mutex_unlock( &dspContext->mutex );
        if ( retval != FADAS_ERROR_NONE )
        {
