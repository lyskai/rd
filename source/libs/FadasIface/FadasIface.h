...
extern "C"
{
     ...
     * @retval, 0 on success
     */
    __QAIC_HEADER_EXPORT int __QAIC_HEADER( FadasIface_open )( const char *uri, remote_handle64 *h )
    ...
    __QAIC_HEADER_EXPORT int
            __QAIC_HEADER( FadasIface_close )( remote_handle64 h ) __QAIC_HEADER_ATTRIBUTE;
    enum FadasIface_FadasRemapPipeline_e
    {
        ...
        FADAS_REMAP_PIPELINE_YUV888_TO_RGB888_NSP,   ///< Convert YUV888 to RGB888 [see
                                                     ///< FadasCvtYUV_UYVYtoRGB()]
        FADAS_REMAP_PIPELINE_UYVY_TO_RGB888_NSP,     ///< Convert UYVY to RGB [see
                                                     ///< FadasCvtYUV_UYVYtoRGB()]
        FADAS_REMAP_PIPELINE_VYUY_TO_RGB888_NSP,     ///< Convert VYUY to RGB
        FADAS_REMAP_PIPELINE_UYVY_TO_RGB888_ROISCALE_NSP,   ///< Convert UYVY to RGB [see
                                                            ///< FadasCvtYUV_UYVYtoRGB()] + ROI
                                                            ///< scaling
        FADAS_REMAP_PIPELINE_VYUY_TO_RGB888_ROISCALE_NSP,   ///< Convert VYUY to RGB + ROI scaling
        FADAS_REMAP_PIPELINE_UYVY_TO_RGB888_NORMI8_NSP,     ///< Convert UYVY to RGB [see
                                                            ///< FadasCvtYUV_UYVYtoRGB()] +
                                                            ///< Renormalization
        FADAS_REMAP_PIPELINE_UYVY_TO_RGB888_NORMU8_NSP,     ///< Convert UYVY to RGB [see
                                                            ///< FadasCvtYUV_UYVYtoRGB()] +
                                                            ///< Renormalization with uint8 output
    ...
    {
        FADAS_CVTYUV_PIPELINE_DownscaleUYVYAndRGB888_NSP,      ///< UYVY to RGB using parameters of
                                                               ///< FadasCvtYUV_UYVYtoRGB()
        FADAS_CVTYUV_PIPELINE_DownscaleUYVYBy2AndRGB888_NSP,   ///< UYVY to RGB using parameters of
                                                               ///< FadasCvtYUV_UYVYtoRGB()
        FADAS_CVTYUV_PIPELINE_DownscaleYUV888AndRGB888_NSP,    ///< 3 8-bit channel YUV to RGB using
                                                               ///< parameters of
                                                               ///< FadasCvtYUV_UYVYtoRGB()
        FADAS_CVTYUV_PIPELINE_DownscaleYUV888By2AndRGB888_NSP,   ///< 3 8-bit channel YUV to RGB
                                                                 ///< using parameters of
                                                                 ///< FadasCvtYUV_UYVYtoRGB()
        FADAS_CVTYUV_PIPELINE_DownscaleY8UV8by2_NSP,     ///< Multi planar image (Y,UV) with 8 bit
                                                         ///< data in 8 bit carriers
        FADAS_CVTYUV_PIPELINE_DownscaleY10UV10by2_NSP,   ///< Multi planar image (Y,UV) with 10 bit
                                                         ///< data in 16 bit carriers
    ...
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasDeInit )( remote_handle64 _h )
            __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasCvtYUV_UYVYtoRGB )(
            remote_handle64 _h, const uint8_t *src, int srcLen,
            const FadasIface_FadasImgProps_t *srcProps, uint8_t *dst, int dstLen,
            uint32_t dstStride ) __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasCvtYUV_UYVYtoYUV )(
            remote_handle64 _h, const uint8_t *src, int srcLen,
            const FadasIface_FadasImgProps_t *srcProps, uint8_t *dst, int dstLen,
            uint32_t dstStride ) __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasCvtYUV_UYVYtoNV12 )(
            remote_handle64 _h, int32_t srcFd, uint32_t srcLen, uint32_t offset,
            const FadasIface_FadasImgProps_t *srcProps, int32_t dstFd, uint32_t dstLen,
            uint32_t dstStride ) __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasCvtYUV_CreateWorkers )(
            remote_handle64 _h, uint64 *addr, int32 num_threads,
            FadasIface_FadasCvtYUVPipeline_e type ) __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasCvtYUV_DestroyWorkers )(
            remote_handle64 _h, uint64 addr ) __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasCvtYUV_RunMT )(
            remote_handle64 _h, uint64 addr, uint8_t *src, int srcLen, uint8_t *dst, int dstLen,
            FadasIface_FadasImgProps_t *srcProps, FadasIface_FadasImgProps_t *dstProps,
            FadasIface_FadasROI_t *roi ) __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasRemap_CreateMapFromMap )(
            ...
            uint32_t mapWidth, uint32_t mapHeight, const float *mapX, int mapXLen,
            const float *mapY, int mapYLen, uint32_t mapStride,
            FadasIface_FadasRemapPipeline_e imgFormat,
            uint8_t borderConst ) __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasRemap_CreateMapFromFisheyeCalib )(
            remote_handle64 _h, uint64 *mapPtr, uint32_t camWidth, uint32_t camHeight,
            const FadasIface_FadasCameraProps_t *camProps,
            const FadasIface_FadasDistCoeffs_t *distCoeffs,
            FadasIface_FadasRemapPipeline_e imgFormat, uint32_t mapWidth, uint32_t mapHeight,
            uint8_t borderConst ) __QAIC_HEADER_ATTRIBUTE;
    ...
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasRemap_Run )(
            remote_handle64 _h, uint64 mapPtr, const uint8_t *src, int srcLen,
            const FadasIface_FadasImgProps_t *srcProps, uint8_t *dst, int dstLen,
            const FadasIface_FadasImgProps_t *dstProps,
            FadasIface_FadasROI_t *dstROI ) __QAIC_HEADER_ATTRIBUTE;
    ...
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasCvtYUV_Renormalize888 )(
            remote_handle64 _h, int32_t srcFd, const FadasIface_FadasImgProps_t *srcProps,
            const FadasIface_FadasNormlzParams_t *normlzR,
            const FadasIface_FadasNormlzParams_t *normlzG,
            const FadasIface_FadasNormlzParams_t *normlzB, int32_t dstFd, uint32_t dstStride,
            int32_t batchSize ) __QAIC_HEADER_ATTRIBUTE;
    __QAIC_HEADER_EXPORT AEEResult __QAIC_HEADER( FadasIface_FadasCvtYUV_Renormalize888u8f32 )(
            remote_handle64 _h, int32_t srcFd, const FadasIface_FadasImgProps_t *srcProps,
            const FadasIface_FadasNormlzParams_t *normlzR,
            const FadasIface_FadasNormlzParams_t *normlzG,
            const FadasIface_FadasNormlzParams_t *normlzB, int32_t dstFd, uint32_t dstStride,
